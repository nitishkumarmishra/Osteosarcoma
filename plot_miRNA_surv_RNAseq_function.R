...
plot_surv.Protein <- function (dir, clinical_patient, dataGE, Genelist, Survresult = FALSE, Median = TRUE, Mean =TRUE, Sdev=0, p.cut = 0.05) 
{
  ...
  if (!is.null(Genelist)) {
    ...
    group2 <- colnames(dataGE[,grep("TARGET", colnames(dataGE))])
    dataCancer <- dataGE[Genelist, group2, drop = FALSE]
    colnames(dataCancer) <- substr(colnames(dataCancer), 1, 16)
  }
  cfu <- OS.Clinical.TARGET[ OS.Clinical.TARGET[, "TARGET USI"] %in% substr(colnames(OS.miRNA), 1, 16), ]
  ...
  for (i in 1:ngenes) {
    ...
    if (!is.na(mRNAselected_values_ordered_top)) {
      ...
      tabSurv_pvalue <- tryCatch({
        ...
        print(paste0("Now running for the miRNA :: ",mRNAselected, "  Survdiff P-value = ", pvalue_survdiff, "  survminer P-value = ", pvalue))
      ...
      if (Survresult == TRUE & pvalue1 <= p.cut) {
        ...
        text(70.5, 0.98, paste0("miRNA (",mRNAselected,")"), cex = 1.25, col = "blue4")
        legend("topright", bty="n", x.intersp=0.1, bg='lightblue', inset = 0, y.intersp=0.75, lty=1, lwd=1, cex=1.25, text.font=1, legend = c(legendHigh, legendLow), col = c("red4","blue4"), text.col = c("red4","blue4"), pch = 15)
  ...
  return(tabSurvKM)
  #write.csv(tabSurvKM, file = 'result.txt')
}
#####################