library(survival)
library(survminer)
plot_surv.lncRNA <- function (dir, clinical_patient, dataGE, Genelist, Survresult = FALSE, Median = TRUE, Mean =TRUE, Sdev=0, p.cut = 0.05) 
{
  ...
  for (i in 1:ngenes) {
    ...
    if (!is.na(mRNAselected_values_ordered_top)) {
      ...
      if (Survresult == TRUE & pvalue1 <= p.cut) {
        ...
        text(60.5, 0.98, paste0("lncRNA (",mRNAselected,")"), cex = 1.5, col = "blue4")
