dichotomize <- function(x, cutpoint, labels = c("Loss", "Gain", "Neutral")){
  grps <- x
  grps[x <= -cutpoint] = labels[1]
  grps[x >= cutpoint] = labels[2]
  grps[x > -cutpoint & x <cutpoint] = labels[3]
  return(grps)
}

GISTIC_Gene_Arm_Level_Threshold <- dichotomize(GISTIC_Gene_Arm_Level, cutpoint = 0.3)
...
univ_results <- lapply(univ_models,
                       function(x){ 
                         ...
                         names(res)<-c("Beta", "HR (95% CI)", "Wald test", "Wald P-value")
                         return(res)
...
univ_formulas <- sapply(covariates,
                        function(x) surv_fit(as.formula(paste('Surv(time, status)~', x)), data=lung))

univ_results <- lapply(univ_formulas,function(x){
  x <- surv_pvalue(x)
  y <- unlist(x)
  p.value <- signif(as.numeric(y[2]), digits=2)
  variable <- y[1]
  res<-c(variable, p.value)
  names(res) <-c("Variable", "LR P.val")
  return(res)
})

res.log.rank <- t(as.data.frame(univ_results, check.names = FALSE))
res.log.rank <- as.data.frame(res.log.rank)
results <- dplyr::select(transform(merge(res.log.rank, res, by.y="row.names", by.x="Variable"), row.names=Variable, check.names = FALSE), -Variable)
...
for(i in 1:length(genes)){
  ...
  print(
    ggsurvplot(fit,
               ...
               title = paste0(genes[i], "survival plot"),
...
#covariates <- colnames(Surv_CNV.Data[3:ncol(Surv_CNV.Data)])
covariates <- colnames(Surv_CNV.Data[3:10])
univ_formulas <- sapply(covariates,
                        function(x) as.formula(paste('Surv(Time, Event)~', x)))

univ_models <- lapply( univ_formulas, function(x){coxph(x, data = Surv_CNV.Data)})
# Extract data 
univ_results <- lapply(univ_models,
                       function(x){ 
                         x <- summary(x)
