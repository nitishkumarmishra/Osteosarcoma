library(survival)
library(survminer)
plot_surv <- function (dir, clinical_patient, dataGE, Genelist, Survresult = FALSE, Median = TRUE, Mean =TRUE, Sdev=0, p.cut = 0.05) 
{
  ...
  cfu <- OS.Clinical.TARGET[ OS.Clinical.TARGET[, "TARGET USI"] %in% substr(colnames(OS.FPKM.UQ), 1, 16), ]
